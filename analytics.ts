...
export async function getAnalyticsContext() {
  ...
  return { traceId, contextId, signedContextId: signContextId(contextId), projectId: "cmu988ag500pvuap9811hrnbk" };
}
...
export async function logPageActivity(contextId: string, pageUrl: string): Promise<void> {
  ...
  try {
    await fetch("https://neupgroup.com/analytics/bridge/api.v1/activity?project=cmu988ag500pvuap9811hrnbk", {
      method: "POST",
...
export async function logActivity(activity: string, data?: Record<string, unknown>): Promise<void> {
  ...
  const requestHeaders = await headers();
  await fetch(
    "https://neupgroup.com/analytics/bridge/api.v1/activity?project=cmu988ag500pvuap9811hrnbk",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
